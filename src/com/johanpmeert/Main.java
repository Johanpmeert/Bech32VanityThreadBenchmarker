...
public class Main {
    ...
    public static String vanityString = "jmeert";
    public static volatile long bech32Counter;
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("-----------------------------------");
        System.out.println("Looking for " + vanityString + " with " + numberOfThreads + " threads");
        ...
        Runnable vanityThread = new VanityGenThread();
        for (int teller = 0; teller < numberOfThreads; teller++) {
            new Thread(vanityThread).start();
        }
    }

    public static class VanityGenThread implements Runnable {
        ...
        public void run() {
            ...
            while (!stopCalled) {
                ...
                if (bech32String.contains(vanityString)) {
                    ...
                    System.out.println("Speed: " + bech32Counter * 1.0e9 / (stoptime - starttime) + " addresses/sec");
                    System.out.println("Executed on " + numberOfThreads + " threads");
                    stopCalled = true;
                    return;
                }
                bech32Counter++;
    ...
    public static String hexString8To5bit(String convert) {
        ...
        String binaryString = new BigInteger(convert, 16).toString(2); // convert our hex String to binary
        int bitCounter = (int) (5.0 * Math.ceil(binaryString.length() / 5.0));
        while (binaryString.length() < bitCounter) {
            binaryString = "0" + binaryString; // make sure that the string has multiple of 5 in number of bits, padding with 0 in front if necessary
        }
        ...
        for (int teller = 0; teller < (bitCounter / 5); teller++) {
            ...
            binaryString = binaryString.substring(5); // erase those 5 bits from remainder
